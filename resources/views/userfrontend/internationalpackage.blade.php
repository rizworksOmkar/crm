...
@section('styles')
    @if ($destinationbanner)
        <style>
            #common_banner {
                background-image: url({{ $destinationbanner }});
                                        ...
                                        @foreach ($activity as $activityitem)
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox"
                                                    value="{{ $activityitem->id }}" name="activity_type_id[]"
                                                        ...
                                                        <div class="cruise_content_rate">
                                                            ...
                                                            <div class="cruise_content_box_btm">
                                                                ...
                                                                <div class="cruise_content_middel_right">
                                                                    ...
                                                                    <h3><i class="fas fa-rupee-sign fa-xs"></i>
                                                                        {{ getPacakgeCostByseason($item->id) }}
                                                                        <sup><span>/</span> Per
                                                                            person</sup>
                                                                    </h3>
...
@section('scripts')
    <script>
        $(function() {
            $('input[name="for_number_of_nights"], input[name="rdoBudget"], input[name="activity_type_id[]"]').on('change', function() {
                $.ajax({
                    url: '/filter-packages',
                    method: 'POST',
                    ...
                    success: function(response) {
                        $('#packages').empty();
                        $('#packCount').empty();
                        console.log(response.pacakages);
                        $('#packCount').append(
                            '<h2>' + response.pacakageCount + ' Packages found</h2>'
                        );

                        //code to get country name upon filter
                        var countryNames = response.countryNames;
                        //var packageTypes = response.packageTypesName;

                        $.each(response.pacakages, function(key, item) {

                            //also have to do this to get them all
                            var packageCountriesList = response.packageCountries[item
                                .id] || '';
                            //var packageType = packageTypes[item.package_type_id] || '';

                            var packageActivitiesList = response.packageActivities[item
                                .id] || '';
